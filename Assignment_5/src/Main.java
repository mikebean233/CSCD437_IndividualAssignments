...
public class Main implements Runnable {
    ...
    Hashtable<String, RegexMenuItem> regexMap = new Hashtable<> ();
    ...
    public void run(){
        regexMap.put("a", new RegexMenuItem("^[0-9]{3}(\\s|-|())[0-9]{2}\\1[0-9]{4}$"                                        ,"Social Security Number"));
        regexMap.put("b", new RegexMenuItem("^(\\([0-9]{3}\\)|[0-9]{3})(\\s|())[0-9]{3}(\\s|()|-)[0-9]{4}$"                  ,"US Phone number"));
        regexMap.put("c", new RegexMenuItem("^([a-zA-Z0-9]|_)*@([a-zA-Z0-9]|_)*\\.[a-zA-Z]{2,3}$"                            ,"E-mail address"));
        regexMap.put("d", new RegexMenuItem("^[A-Z][a-z]+,\\s[A-Z][a-z]+,\\s[A-Z]+$"                                         ,"Name on a class roster, assuming one or more middle initials - Last name, First name, MI"));
        regexMap.put("e", new RegexMenuItem("^(0[1-9]|1[012])-(0[1-9]|[12][0-9]|3[01])-[0-9]{4}$"                            ,"Date in MM-DD-YYYY format"));
        regexMap.put("f", new RegexMenuItem("^[1-9][0-9]+\\s[A-Z][a-z]*\\s(?i)(street|road|boulevard|avenue|st|rd|blvd|ave)$","House address - Street number, street name, abbreviation for road, street, boulevard or avenue"));
        regexMap.put("g", new RegexMenuItem("^([A-Z][a-z]{2,}\\s)*([A-Z][a-z]{2,}|A[LKZRS]|C[AOT]|D[CE]|F[LM]|G[AU]|I[DLNA]|K[SY]|LA|M[EDAINHPSOT]|N[EVHJMYCD]|O[HKR]|P[AWR]|RI|S[CD]|T[NX]|UT|V[ITA]|W[AVIY])\\s\\d{5}(-\\d{4})?$","City followed by state followed by zip as it should appear on a letter"));
        regexMap.put("h", new RegexMenuItem("^([01]\\d|2[0-3])[0-5]\\d:[0-5]\\d$"                                            ,"Military time, including seconds"));
        regexMap.put("i", new RegexMenuItem("^\\$[^,0]([1-9]|[1-9]\\d|[1-9]\\d\\d|())((?<comma>(,|()))\\d{3})?(\\k<comma>\\d{3})*\\.\\d{2}$","US Currency down to the penny (ex: $123,456,789.23)"));
        regexMap.put("j", new RegexMenuItem("^http:\\/\\/([^\\/\\s]*\\/[^\\/\\s]*)*"                                         ,"URL, including http:// (upper and lower case should be accepted)"));
        regexMap.put("j", new RegexMenuItem(
            new StringVerifier[]{
                new StringVerifier("[a-zA-Z]", new Condition[]{new Condition(10, Operator.GreaterThenOrEqualTo)}),
        ...
        regexMap.put("l", new RegexMenuItem(
            new StringVerifier[]{
        ...
        do{
            ...
            if(regexMap.containsKey(menuChoice)){
                RegexMenuItem thisRegexMenuItem = regexMap.get(menuChoice);
    ...
    public void processRegexItem(String inputString, RegexMenuItem regexMenuItem){
      ...
      //System.out.println("regex:                     " + regexMenuItem.getRegex());
    ...
    public void printMenu(){
        ...
        String[] keys = this.regexMap.keySet().toArray(new String[]{});
        Arrays.sort(keys);

        for(String thisKey : keys){
            System.out.println(thisKey + ". " + this.regexMap.get(thisKey).getDescription());
        }
